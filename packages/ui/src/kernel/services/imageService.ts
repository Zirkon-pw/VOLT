...
export interface ImageServiceMethods {
  copyImage: (locator: string, sourcePath: string, imageDir: string) => Promise<string>;
  saveImageBase64: (locator: string, fileName: string, imageDir: string, base64Data: string) => Promise<string>;
  pickImage: () => Promise<string>;
  readImageBase64: (locator: string, relPath: string) => Promise<string>;
...
export function copyImage(locator: string, sourcePath: string, imageDir: string): Promise<string> {
  const m = useImageService.getState().methods;
  if (!m) throw new Error('ImageService not registered');
  return m.copyImage(locator, sourcePath, imageDir);
}

export function saveImageBase64(locator: string, fileName: string, imageDir: string, base64Data: string): Promise<string> {
  const m = useImageService.getState().methods;
  if (!m) throw new Error('ImageService not registered');
  return m.saveImageBase64(locator, fileName, imageDir, base64Data);
}
...
export function readImageBase64(locator: string, relPath: string): Promise<string> {
  const m = useImageService.getState().methods;
  if (!m) throw new Error('ImageService not registered');
  return m.readImageBase64(locator, relPath);
}
