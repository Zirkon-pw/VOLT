...
import { resolvePluginCompatibility } from '@kernel/plugin-system/api/pluginCompatibility';
...
function createWorkspaceInfo(locator: string): PluginWorkspaceInfo {
  const workspaceStore = useWorkspaceStore.getState();
  const activeWorkspaceId = workspaceStore.activeWorkspaceId ?? '';
  const activeWorkspace = workspaceStore.workspaces.find((workspace) => workspace.id === activeWorkspaceId);

  return {
    workspaceId: activeWorkspace?.id ?? activeWorkspaceId,
    displayName: activeWorkspace?.displayName ?? '',
    voltId: activeWorkspace?.id ?? activeWorkspaceId,
    locator,
    rootPath: locator,
  };
}
...
async function registerLoadedRuntime(
  ...
  if (lifecycle.onWorkspaceOpen) {
    await safeExecuteAsync(pluginId, 'onWorkspaceOpen', async () => lifecycle.onWorkspaceOpen!(api, createWorkspaceInfo(locator)));
  }
}
...
async function loadBuiltinPlugin(pluginId: string, locator: string, sessionId?: number): Promise<void> {
  const plugin = getBuiltinPlugin(pluginId);
  if (!plugin) {
    return;
  }

  const compatibility = resolvePluginCompatibility(plugin.manifest);
  if (!compatibility.supported) {
    usePluginLogStore.getState().addEntry(
      plugin.manifest.id,
      'warn',
      `Skipped on this platform: ${compatibility.reason ?? 'unsupported runtime'}`,
    );
    return;
  }
  ...
  await registerLoadedRuntime(plugin.manifest.id, builtinPluginRuntimes, api, plugin, locator);
...
export async function loadPlugin(
  ...
  if (!pluginInfo.compatibility.supported) {
    usePluginLogStore.getState().addEntry(
      pluginId,
      'warn',
      `Skipped on this platform: ${pluginInfo.compatibility.reason ?? 'unsupported runtime'}`,
    );
    return;
  }
  ...
  try {
    ...
    await registerLoadedRuntime(pluginId, externalPluginRuntimes, api, lifecycle, locator);
...
export async function loadAllPlugins(locator: string): Promise<void> {
  ...
  try {
    for (const plugin of builtinPlugins) {
      ...
      await loadBuiltinPlugin(plugin.manifest.id, locator, sessionId);
    }
    ...
    const enabled = plugins.filter((plugin) => plugin.enabled && plugin.compatibility.supported);
    ...
    for (const plugin of enabled) {
      ...
      await loadPlugin(plugin, locator, sessionId);
    }
  } catch (err) {
...
export async function loadSinglePlugin(pluginId: string, locator: string): Promise<void> {
  try {
    unloadSinglePlugin(pluginId);
    if (isBuiltinPlugin(pluginId)) {
      await loadBuiltinPlugin(pluginId, locator);
      return;
    }
    ...
    await loadPlugin(pluginInfo, locator);
  } catch (err) {
