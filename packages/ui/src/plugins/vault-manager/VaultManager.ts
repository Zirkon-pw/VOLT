...
export async function createVoltInParent(name: string, parentPath: string, directoryName: string): Promise<Volt> {
  ...
  const fullPath = joinPath(parentPath, normalizedDirectoryName);
  const adapter = getCurrentPlatformAdapter();

  // Web adapter provides workspaceCreate which atomically creates the
  // directory on the server and registers the workspace record.
  if (adapter.workspaceCreate) {
    const result = await adapter.workspaceCreate(name.trim() || normalizedDirectoryName, fullPath);
    return {
      id: result.id,
      name: result.name,
      path: result.path,
  ...
  // Desktop flow: create the directory via the file adapter, then persist
  // the workspace record through the storage adapter.
  await createDirectory(parentPath, normalizedDirectoryName);
  return createVolt(name, fullPath);
}
