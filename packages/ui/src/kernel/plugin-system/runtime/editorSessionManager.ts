...
function queueSessionSave(session: InternalEditorSession): Promise<void> {
  session.saveChain = session.saveChain
    .catch(() => undefined)
    .then(async () => {
      await writeFile(session.locator, session.filePath, session.markdown);
    });
...
function syncSessionAttachment(session: InternalEditorSession): void {
  ...
  const shouldAttach = Boolean(
    editorState.editor &&
    editorState.locator === session.locator &&
...
export async function captureActiveEditorSession(
  ...
  const editorState = getEditorState();
  if (!editorState.filePath || editorState.locator !== locator) {
    return null;
  }

  return openEditorSession(pluginId, locator, editorState.filePath);
}
...
export function getEditorSessionSourceInfo(
  sessionId: string,
): { locator: string; filePath: string } | null {
