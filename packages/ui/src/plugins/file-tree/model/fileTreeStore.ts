...
interface FileTreeState {
  ...
  loadTree: (voltId: string, locator: string) => Promise<void>;
  refreshTree: (voltId: string, locator: string) => Promise<void>;
  notifyFsMutation: (voltId: string, locator: string) => Promise<void>;
  ...
  commitInlineEdit: (voltId: string, locator: string) => Promise<string | null>;
  cancelInlineEdit: (voltId: string) => void;
  requestDelete: (voltId: string, path: string) => void;
  cancelDelete: (voltId: string) => void;
  confirmDelete: (voltId: string, locator: string) => Promise<void>;
  ...
  commitMove: (voltId: string, locator: string) => Promise<string | null>;
...
async function loadTreeData(set: FileTreeSetState, voltId: string, locator: string) {
  set((state) => ({
  ...
  try {
    const tree = await listTree(locator);
    set((state) => ({
...
export const useFileTreeStore = create<FileTreeState>((set, get) => ({
  ...
  loadTree: async (voltId, locator) => {
    await loadTreeData(set, voltId, locator);
  },

  refreshTree: async (voltId, locator) => {
    await loadTreeData(set, voltId, locator);
  },

  notifyFsMutation: async (voltId, locator) => {
    try {
      await get().refreshTree(voltId, locator);
    } catch (err) {
  ...
  commitInlineEdit: async (voltId, locator) => {
    ...
    if (pendingCreate) {
      ...
      const nextPath = joinRelativePath(pendingCreate.parentPath, normalizedName);
        ...
        if (pendingCreate.isDir) {
          await createDirectory(locator, nextPath);
          showSuccess(translate('fileTree.toast.folderCreated', { name: normalizedName }));
        } else {
          await createMarkdownFile(locator, nextPath);
    ...
    try {
      await renamePath(locator, editingItem.path, nextPath);
  ...
  confirmDelete: async (voltId, locator) => {
    ...
    try {
      await deletePath(locator, target.path);
  ...
  commitMove: async (voltId, locator) => {
    ...
    try {
      await renamePath(locator, draggingPath, nextPath);
