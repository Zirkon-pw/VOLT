...
import { resolvePluginCompatibility } from './pluginCompatibility';
...
function isPluginManifest(value: unknown): value is PluginManifest {
  ...
  return (
    ...
    && (manifest.platforms == null || (Array.isArray(manifest.platforms) && manifest.platforms.every((entry) => typeof entry === 'string')))
    && (manifest.requiredCapabilities == null || (Array.isArray(manifest.requiredCapabilities) && manifest.requiredCapabilities.every((entry) => typeof entry === 'string')))
  );
}
...
export const WEB_STORAGE_SENTINEL = 'web:storage';

async function ensurePluginsDirectory(): Promise<string> {
  const configDir = await getStorageConfigDir();

  if (configDir === WEB_STORAGE_SENTINEL) {
    return WEB_STORAGE_SENTINEL;
  }
...
export async function listPlugins(): Promise<PluginInfo[]> {
  ...
  if (pluginsDir === WEB_STORAGE_SENTINEL) {
    ...
    .map(async (entry) => {
      try {
        ...
        return {
          ...
          compatibility: resolvePluginCompatibility(manifest),
