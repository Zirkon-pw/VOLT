...
function MarkdownEditorDriver({
  ...
  const { resolve, register, unresolveAll, resolveAll, clear } = useImageResolver(locator);
  ...
  const saveEmbedded = useCallback(async () => {
    ...
    await writeFile(locator, filePath, markdown);
    setEmbeddedDirty(false);
    controller.setDirty(false);
    controller.emit('save', undefined);
  }, [controller, editor, filePath, unresolveAll, locator]);
  ...
  useEffect(() => {
    ...
    if (filePath) {
      setEditor(editor, { voltId: voltId ?? '', locator, filePath });
      return () => {
        setEditor(null);
      };
    }
    return undefined;
  }, [editor, filePath, mode, voltId, locator]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        clear();
        const raw = await readFile(locator, filePath);
        if (cancelled) {
          return;
        }
...
function RawTextEditorDriver({
  ...
  const saveFileTab = useCallback(async () => {
    ...
    await writeFile(locator, filePath, valueRef.current);
    setDirty(voltId, filePath, false);
    controller.setDirty(false);
    controller.emit('save', undefined);
    emit('file-save', filePath);
  }, [controller, filePath, setDirty, voltId, locator]);
  ...
  }, [controller, filePath, mode, setDirty, voltId, locator]);
  ...
  return (
    <div style={{ display: 'flex', flex: 1, minHeight: 0, minWidth: 0, flexDirection: 'column' }}>
      {mode === 'file-tab' && <PluginTaskStatusBanner locator={locator} filePath={filePath} />}
      <textarea
        key={filePath}
        ref={textareaRef}
        style={{
...
function ImageEditorDriver({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const dataUrl = await readImageBase64(locator, filePath);
        if (cancelled) {
          return;
        }
