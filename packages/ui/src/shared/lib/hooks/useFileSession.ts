...
export function useFileSession({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      await writeFile(locator, filePath, content);
      setDirty(voltId, filePath, false);
      emit('file-save', filePath);
    } catch (error) {
      console.error('Failed to save file:', error);
    }
  }, [filePath, getContent, transformBeforeSave, setDirty, voltId, locator]);
  ...
  }, [consumePendingRename, filePath, onClear, pendingRename, save, setContent, setDirty, transformAfterLoad, voltId, locator]);
