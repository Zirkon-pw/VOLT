import type { ProcessEventPayload } from '@common/types';
...
function emitProcessEvent(payload: ProcessEventPayload): void {
...
async function ensureProcessEventBridge(): Promise<void> {
  ...
  if (!eventBridgePromise) {
    eventBridgePromise = (async () => {
      ...
      eventUnsubscribe = await adapter.process.onProcessEvent((payload) => {
