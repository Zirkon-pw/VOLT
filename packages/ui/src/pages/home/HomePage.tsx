...
import { usePlatformAdapter } from '@shared/platform';
...
export function HomePage() {
  ...
  const { capabilities } = usePlatformAdapter();
  const canPickLocalDirectory = capabilities.localWorkspacePath;
  ...
  const validateForm = (): string | null => {
    ...
    if (!selectedPath.trim()) {
      if (modalMode === 'create') {
        return canPickLocalDirectory
          ? t('home.validation.parentRequired')
          : t('home.validation.serverPathRequired');
      }
      return t('home.validation.locationRequired');
    }
  ...
  const handleOpenVolt = (volt: { id: string; name: string; path: string }) => {
    // On web the file API identifies workspaces by UUID (the server resolves
    // the filesystem path internally). On desktop the locator is the raw path.
    const locator = canPickLocalDirectory ? volt.path : volt.id;
    openWorkspace({
      id: volt.id,
      displayName: volt.name,
      locator,
    });
        ...
        {modalMode === 'create' ? (
          <>
            {canPickLocalDirectory ? (
              ...
              <TextInput
                label={t('home.modal.serverPathLabel')}
                placeholder={t('home.modal.serverPathPlaceholder')}
                value={selectedPath}
