...
export interface SearchServiceMethods {
  searchFiles: (locator: string, query: string) => Promise<SearchResult[]>;
}
...
export function searchFiles(locator: string, query: string): Promise<SearchResult[]> {
  const m = useSearchService.getState().methods;
  if (!m) throw new Error('SearchService not registered');
  return m.searchFiles(locator, query);
}
