...
function buildVaultAssetUrl(locator: string, relPath: string): string {
...
export async function copyImage(_locator: string, _sourcePath: string, _imageDir: string): Promise<string> {
...
export async function readImageBase64(locator: string, relPath: string): Promise<string> {
  const response = await fetch(buildVaultAssetUrl(locator, relPath));
  if (!response.ok) {
