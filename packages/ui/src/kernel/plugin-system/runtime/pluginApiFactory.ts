...
async function getWorkspaceEntry(
  ...
  const parentEntries = await listTree(locator, parentPath);
...
export function createPluginAPI(
  ...
  const allKnownPlugins = () => [
    ...builtinPlugins.map((plugin) => ({
      ...
      enabled: true,
      dirPath: 'builtin',
      compatibility: { supported: true, missingCapabilities: [] },
    })),
    ...getCachedPlugins(),
  ];
  ...
  const getWorkspaceInfo = () => {
    ...
    return {
      ...
      displayName: activeWorkspace?.displayName ?? activeWorkspaceId,
      rootPath: locator,
    };
  };
  const notifyFsMutation = async (): Promise<void> => {
    ...
    await getFileTreeServiceStore().notifyFsMutation(voltId, locator);
  };
  ...
  return {
    fs: {
      async read(path: string): Promise<string> {
        ...
        return readFile(locator, normalizePluginPath(path, pluginId, 'fs.read'));
      },
      async write(path: string, content: string): Promise<void> {
        requirePermission('write', 'fs.write');
        return writeFile(locator, normalizePluginPath(path, pluginId, 'fs.write'), content);
      },
      async create(path: string, content = ''): Promise<void> {
        ...
        await createWorkspaceFile(locator, normalizedPath, content);
      ...
      async exists(path: string): Promise<boolean> {
        ...
        try {
          return (await getWorkspaceEntry(locator, pluginId, 'fs.exists', path)) != null;
        } catch {
          return false;
        }
      },
      async stat(path: string) {
        ...
        const entry = await getWorkspaceEntry(locator, pluginId, 'fs.stat', path);
        if (!entry) {
    ...
    workspace: {
      getId(): string {
        requirePermission('read', 'workspace.getId');
        return getWorkspaceInfo().id;
      },
      getDisplayName(): string {
        requirePermission('read', 'workspace.getDisplayName');
        return getWorkspaceInfo().displayName;
      },
      ...
      getRootPath(): string {
        ...
        return getWorkspaceInfo().rootPath;
      },
    },
    search: {
      ...
      query(text: string) {
        ...
        return searchFiles(locator, text);
      },
    },
    assets: {
      pickImage() {
    ...
    process: {
      async start(config) {
        ...
        const handle = await startPluginProcess(pluginId, locator, config);
    ...
    editor: {
      async captureActiveSession() {
        ...
        const session = await captureActiveEditorSession(pluginId, locator);
        return session ? wrapSession(session) : null;
      },
      async openSession(path: string) {
        requirePermission('editor', 'editor.openSession');
        const session = await openEditorSession(pluginId, locator, path);
