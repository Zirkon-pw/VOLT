...
import { resolveFileViewTarget } from '@kernel/plugin-system/runtime/fileViewResolution';
import {
  isRegisteredHostEditorViewerUsable,
  renderHostEditorFileSurface,
} from '@kernel/plugin-system/runtime/hostEditorService';
import { EditorPanel } from '@kernel/editor/ui/EditorPanel';
...
function PluginFileViewerHost({
  ...
  }, [filePath, registerSaveHandler, viewer, voltId, locator]);
...
export function FileViewHost({ voltId, locator, filePath }: FileViewHostProps) {
  ...
  if (!filePath) {
    return <EditorPanel voltId={voltId} locator={locator} filePath={filePath} />;
  }
