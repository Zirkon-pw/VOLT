...
export function EditorPanel({ voltId, locator, filePath }: EditorPanelProps) {
  const { t } = useI18n();
  ...
  const { handleDrop, handleDragOver, handlePaste } = useImageHandlers({
    editor, voltId, locator, filePath, imageDir, resolve, register, notifyFsMutation,
  });
  ...
  useEffect(() => {
    if (editor) {
      setEditor(editor, { voltId, locator, filePath });
    } else {
      setEditor(null);
    }
    return () => { setEditor(null); };
  }, [editor, filePath, voltId, locator]);
