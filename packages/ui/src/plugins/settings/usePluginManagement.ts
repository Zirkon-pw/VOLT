...
import {
  ...
  pickPluginArchive,
  setPluginEnabled,
} from '@kernel/plugin-system/api/catalogApi';
...
import { notifyError, notifyInfo, notifySuccess } from '@shared/ui/toast';
...
export function usePluginManagement() {
  ...
  const applyPluginToggle = useCallback(async (plugin: PluginInfo, nextEnabled: boolean) => {
    ...
    try {
      ...
      if (nextEnabled) {
        if (activeWorkspace?.locator) {
          await PluginLifecycleManager.loadSinglePlugin(pluginId, activeWorkspace.locator);
        }
      } else {
        PluginLifecycleManager.unloadSinglePlugin(pluginId);
      }
    } catch (err) {
  ...
  }, [activeWorkspace?.locator]);

  const handleTogglePlugin = useCallback(async (plugin: PluginInfo) => {
    if (!plugin.enabled && !plugin.compatibility.supported) {
      notifyError(plugin.compatibility.reason ?? t('settings.plugins.incompatible'));
      return;
    }
  ...
  const settingsPlugins = useMemo(
    () => [...plugins]
      ...
      .filter((plugin) => (
        plugin.manifest.settings?.customUI
        || (plugin.manifest.settings?.sections ?? []).length > 0
      ))
