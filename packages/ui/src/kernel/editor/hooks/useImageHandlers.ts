...
export function useImageHandlers({
  ...
  const saveAndInsert = useCallback(async (file: Blob, fileName: string) => {
    try {
      ...
      const relPath = await saveImageBase64(locator, fileName, imageDir, b64);
      const blobUrl = base64ToBlobUrl(b64, file.type);
      await insertImage(relPath, blobUrl);
      void notifyFsMutation(voltId, locator);
    } catch (err) {
      console.error('Failed to save image:', err);
    }
  }, [locator, imageDir, insertImage, notifyFsMutation, voltId]);
  ...
  useEffect(() => {
    ...
    const handler = async () => {
      try {
        ...
        if (selectedPath) {
          const relPath = await copyImage(locator, selectedPath, imageDir);
  ...
  }, [editor, imageDir, insertImage, notifyFsMutation, voltId, locator]);
