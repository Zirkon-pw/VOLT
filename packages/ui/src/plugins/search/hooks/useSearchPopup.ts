...
async function searchPluginOwnedFiles(
  ...
  for (const entry of files) {
    ...
    try {
      const content = await readFile(locator, entry.path);
...
export function useSearchPopup(
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        const [coreResults, pluginResults] = await Promise.all([
          searchFiles(locator, trimmedQuery),
          searchPluginOwnedFiles(locator, tree, normalizedQuery, pluginSearchProviders),
        ]);
  ...
  }, [isCommandMode, isOpen, pluginSearchProviders, query, tree, locator]);
